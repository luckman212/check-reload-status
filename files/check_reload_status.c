 ...
 * explicit FastCGI response saying the request was not completed).
 * Use 1, 2, 4, 8, 16, then 30-second retries.
 */
static void
	...
	TAILQ_FOREACH(tmpcmd, &cmds, rq_link) {
		...
		if (transient_serializable && tmpcmd->transient_serialized &&
			...
			transient_update_pending(tmpcmd, transient_kind,
			                         transient_link_action);
			if (argv != NULL)
				syslog(LOG_INFO, cmd->cmd.syslog, argv);
			else
				syslog(LOG_INFO, "%s", cmd->cmd.syslog);
...
load_config(void)
{
	...
	if (fp == NULL) {
		if (errno != ENOENT)
			syslog(LOG_DEBUG, "Could not open %s: %m", CONFIG_PATH);
		return;
	}
...
main(void)
{
	...
	if ((p = getenv("fcgipath")) != NULL) {
		fcgipath = p;
		syslog(LOG_INFO, "fcgipath from environment %s", fcgipath);
	}
	if ((p = getenv("controlpath")) != NULL) {
		...
		controlpath = p;
		syslog(LOG_INFO, "controlpath from environment %s", controlpath);
	}
	...
	if (!foreground && fork() == 0) {
		...
		while (1) {
			...
			case 1:
				syslog(LOG_NOTICE, "Reloading check_reload_status because it exited from an error!");
	...
	return (0);
error:
	syslog(LOG_INFO, "check_reload_status is stopping.");
